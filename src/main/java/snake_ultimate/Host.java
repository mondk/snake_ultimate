...
try {
			...
			// queuing players 
			while(true) {
			while (players.size()<4) { //cannot start with more than 4 players, get start from host to start
				Object[] p = queue.get(new FormalField(String.class),new FormalField(String.class));
				
				//adds players if they are new else prints their message that is "join"
				if("join".equals(p[1])) {
					...
					if(newPlayer) {
					players.add(new PlayerInfo((String)p[0],(byte) (players.size()+1)));//adds a playerInfo for that new player
